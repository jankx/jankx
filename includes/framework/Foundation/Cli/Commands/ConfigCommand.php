...
class ConfigCommand extends WP_CLI_Command
{
    ...
    {
        try {
            // Special handling for package.json - write content directly instead of copying
            if ($buildFile === 'package.json') {
                $content = $this->getPackageJsonContent();

                // Write to child file without header comment (JSON doesn't support comments)
                $result = file_put_contents($childFile, $content);
            ...
            // For other build files, use original logic
            $content = file_get_contents($parentFile);
            if ($content === false) {
                return false;
            }
    ...
    /**
     * Get package.json content for child theme
     *
     * @return string
     */
