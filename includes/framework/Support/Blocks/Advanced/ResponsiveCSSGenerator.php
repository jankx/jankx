...
class ResponsiveCSSGenerator
{
    ...
    {
        wp_enqueue_style(
            'cheephub-responsive-blocks-frontend',
            get_template_directory_uri() . '/resources/advanced-blocks/build/frontend.css',
            [],
            '1.0.0'
        );
    }
    ...
    protected function generateCSSForType($block, $type, $deviceMappings, $attributes)
    {
        ...
        foreach ($deviceMappings as $device => $attributeName) {
            if ($device === 'desktop') {
                continue; // Skip desktop as it uses default attributes
            }
