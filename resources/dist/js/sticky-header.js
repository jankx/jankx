...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const stickyLogoUrl = (headerOptions.sticky_header_logo_url || '').trim();
    if (!isStickyEnabled) {
        return;
    }
     ...
     */
    const findLogoImg = () => {
        return (header.querySelector('.wp-block-site-logo img, .site-logo img, .custom-logo, img.custom-logo, header .site-branding img, header .navbar-brand img') || null);
    };
    /**
     * Swap to the sticky logo.
     * Saves the original src in a data attribute so it can be restored later.
     */
    const applyLogo = (img, url) => {
