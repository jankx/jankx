...
class AdvancedFilters {
                        ...
                        if (node.nodeType === Node.ELEMENT_NODE) {
                            ...
                            const newOrdering = element.querySelector?.('.woocommerce-ordering .orderby') ||
                                (element.classList?.contains('woocommerce-ordering') ?
                                    element.querySelector('.orderby') : null);
            ...
            const updatePromises = this.config.targetBlockIds.map(async (blockId) => {
                ...
                if (targetBlock) {
                    ...
                    if (blockSettings) {
                        ...
                        if (postsPerPage)
                            attributes.postsPerPage = parseInt(postsPerPage, 10);
                        const columns = targetBlock.getAttribute('data-columns');
                        if (columns)
                            attributes.columns = parseInt(columns, 10);
                        const columnsTablet = targetBlock.getAttribute('data-columns-tablet');
                        if (columnsTablet)
                            attributes.columnsTablet = parseInt(columnsTablet, 10);
                        const columnsMobile = targetBlock.getAttribute('data-columns-mobile');
                        if (columnsMobile)
                            attributes.columnsMobile = parseInt(columnsMobile, 10);
        ...
        Object.entries(this.currentFilters).forEach(([key, value]) => {
            if (Array.isArray(value)) {
                ...
                Object.entries(value).forEach(([k, v]) => {
                    if (v)
                        url.searchParams.set(`${key}_${k}`, String(v));
                });
            }
            else {
    ...
    showLoading() {
        if (!this.config || !this.config.targetBlockIds)
            return;
