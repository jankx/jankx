...
export default function InserterModal(props) {
    ...
    const iconsAll = useMemo(() => flattenIconsArray(iconsByType), [iconsByType]);
    ...
    const getFilteredIcons = useCallback(() => {
        ...
        if (currentCategory.startsWith('all__')) {
            ...
            return (iconsByType.find((type) => type.type === categoryType)
                ?.icons || []);
        }
    ...
    return (_jsx(Modal, { className: "wp-block-outermost-icon-inserter__modal", title: __('Icon Library', 'icon-block'), onRequestClose: () => setInserterOpen(false), isFullScreen: true, children: _jsxs("div", { className: classnames('icon-inserter', {
                'is-searching': searchInput,
            }), children: [_jsx(Sidebar, { iconsByType: iconsByType, currentCategory: currentCategory, onClickCategory: onClickCategory, searchInput: searchInput, setSearchInput: setSearchInput }), _jsxs("div", { className: "icon-inserter__content", children: [_jsx(ContentHeader, { searchInput: searchInput, shownIconsCount: getFilteredIcons().length, iconSize: iconSize, setIconSize: setIconSize }), _jsx(IconGrid, { shownIcons: getFilteredIcons(), iconSize: iconSize, updateIconAtts: updateIconAtts, attributes: attributes })] })] }) }));
}
