...
function handleTabNavigationClick(event) {
    ...
    if (tabBlock) {
        ...
        const tabBlockAttributes = tabBlock && tabBlock.attributes ?
            Array.from(tabBlock.attributes).map(attr => ({
                name: attr.name,
                value: attr.value,
            })) :
            ...
            tabBlock = {
                getAttribute: (attr) => {
                    ...
                    if (attr === 'data-trigger-settings') {
                        ...
                        const targetBlockId = filterBlock.getAttribute('data-target-block-id') ||
                            filterBlock.closest('[data-block-id]')?.getAttribute('data-block-id');
                        if (targetBlockId) {
                            return JSON.stringify({ targetBlockId });
                        }
                    }
...
function extractFilterDataFromBlock(filterBlock, filterType) {
    ...
    const hasRequiredData = (filterType === 'taxonomy' && data.taxonomy) ||
...
function fetchDynamicDataLayout(targetBlockId, filtersPayload) {
    ...
    let targetBlock = document.querySelector(`.wp-block-jankx-dynamic-data-layout[data-block-id="${targetBlockId}"], ` +
    ...
    console.log('[SmartTab AdvancedFilter] Target block search result', {
        ...
        targetBlock,
        searchMethods: {
            byClassAndData: !!document.querySelector(`.wp-block-jankx-dynamic-data-layout[data-block-id="${targetBlockId}"], ` +
                `.wp-block-jankx-dynamic-data-layout[data-query-id="${targetBlockId}"], ` +
                `.wp-block-jankx-dynamic-ssr-layout[data-block-id="${targetBlockId}"], ` +
                `.wp-block-jankx-dynamic-ssr-layout[data-query-id="${targetBlockId}"]`),
