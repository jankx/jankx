...
/******/ (() => {
    ...
    function triggerAdvancedFilterFromBlock(targetBlockId, filterType, filterData) {
        ...
        if (Object.keys(filtersPayload).length === 0) {
            console.warn('AdvancedFilter: Empty filters payload', {
                filterType,
                filterData
            });
            return;
    ...
    function applyFilterToBlock(filterBlock, filterId, values) {
        ...
        if (filterType === 'taxonomy') {
            ...
            if (values.filterValue) {
                ...
                if (!elementFound) {
                    console.warn(`AdvancedFilter: Could not find term option with value ${values.filterValue} for taxonomy ${taxonomy}. Available values:`, Array.from(filterGroup.querySelectorAll('[data-value], input[value]')).map(el => el.getAttribute('data-value') || el.getAttribute('value')));
                }
            }
            else {
    ...
    function buildFiltersPayload(filterId, values) {
        ...
        if (type === 'taxonomy') {
            ...
            if (!taxonomy)
                return null;
            return {
                [taxonomy]: values.filterValue ? [values.filterValue] : []
            };
        }
        if (type === 'meta') {
        ...
        if (type === 'price') {
            return {
                price: {
                    min: values.filterValueMin || '',
                    max: values.filterValueMax || ''
                }
            };
        }
        if (type === 'date') {
            return {
                date: {
                    start: values.filterValueStart || '',
                    end: values.filterValueEnd || ''
                }
            };
        }
        if (type === 'author') {
            return {
                author: values.filterValue ? [values.filterValue] : []
            };
        }
        if (type === 'keyword') {
            return {
                keyword: values.filterValue || ''
            };
        }
        return null;
    }
