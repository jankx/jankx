 ...
 */
import { RichText, useBlockProps, InnerBlocks, __experimentalGetBorderClassesAndStyles as getBorderClassesAndStyles, __experimentalGetShadowClassesAndStyles as getShadowClassesAndStyles, } from '@wordpress/block-editor';
import { validateBlockContent } from './validationUtils';
export default function save({ attributes, className }) {
    const { url, alt, title, id, width, height, aspectRatio, scale, href, linkTarget, rel, caption, showOverlayOnHover, overlayAnimation, overlayAnimationDuration, overlayAnimationDelay, overlayPosition, overlayBackground, overlayOpacity, imageHoverEffect, borderRadius, preset } = attributes;
    ...
    const imageClasses = clsx('wp-block-jankx-advanced-image-box__image', borderProps.className, {
        [`wp-image-${id}`]: !!id,
    ...
    const imageElement = hasImage ? (_jsx("img", { src: url, alt: alt || '', title: title, className: imageClasses, style: imageStyle })) : (
    ...
    _jsx("div", { className: clsx('wp-block-jankx-advanced-image-box__no-image', imageClasses), style: {
            // Keep border and shadow styles, but omit borderRadius if present
            ...(() => {
                const tempStyle = { ...borderProps.style, ...shadowProps.style };
            ...
            width,
            height,
        }, children: !hasImage && alt && (_jsx("div", { className: "wp-block-jankx-advanced-image-box__no-image__alt", style: { color: String(presetOptions.titleColor ?? '#ffffff') }, children: alt })) }));
    ...
    const innerBlocksContent = (_jsx("div", { className: "wp-block-jankx-advanced-image-box__overlay__content", children: _jsx(InnerBlocks.Content, {}) }));
    // Create overlay content if enabled (but not when preset is active)
    const overlayContent = showOverlayOnHover && !preset && (_jsx("div", { className: clsx('wp-block-jankx-advanced-image-box__overlay', `wp-block-jankx-advanced-image-box__overlay--${overlayPosition}`, 'animated', overlayAnimation), style: {
    ...
    const blockClasses = clsx(className, 'wp-block-jankx-advanced-image-box', {
        'has-overlay': showOverlayOnHover,
        'has-hover-effect': imageHoverEffect && imageHoverEffect !== 'none',
    });
    return (_jsxs("figure", { ...useBlockProps.save({ className: blockClasses }), children: [wrappedImage, overlayContent, hiddenContent, _jsx("div", { className: "wp-block-jankx-advanced-image-box__serialized-content", style: { display: 'none' }, children: _jsx(InnerBlocks.Content, {}) }), caption && !RichText.isEmpty(caption) && (_jsx(RichText.Content, { className: "wp-block-jankx-advanced-image-box__caption", tagName: "figcaption", value: caption }))] }));
}
