...
import { BlockControls, InspectorControls, MediaReplaceFlow, RichText, useBlockProps, InnerBlocks, __experimentalUseBorderProps as useBorderProps, __experimentalLinkControl as LinkControl, __experimentalGetShadowClassesAndStyles as getShadowClassesAndStyles, store as blockEditorStore, useBlockEditingMode, } from '@wordpress/block-editor';
import { useSelect, useDispatch } from '@wordpress/data';
import { link, linkOff } from '@wordpress/icons';
import { isBlobURL } from '@wordpress/blob';
import { PanelBody, SelectControl, RangeControl, ToggleControl, TextControl, 
// Use ColorPalette to leverage Gutenberg theme color settings
ColorPicker, ColorPalette, Button, ToolbarGroup, ToolbarButton, Popover, } from '@wordpress/components';
...
` : '';
    ...
    const wrappedImage = href ? (_jsx("a", { href: href, target: linkTarget, rel: rel, className: "wp-block-jankx-advanced-image-box__link", onClick: (e) => e.preventDefault(), children: imageElement })) : (imageElement);
    ...
    return (_jsxs(_Fragment, { children: [combinedPresetCSS && (_jsx("style", { dangerouslySetInnerHTML: { __html: combinedPresetCSS } })), _jsxs("figure", { ...blockProps, children: [wrappedImage, overlayContent, presetContent, hiddenInnerBlocks, !RichText.isEmpty(caption) && (_jsx(RichText, { className: "wp-block-jankx-advanced-image-box__caption", tagName: "figcaption", value: caption, onChange: (value) => setAttributes({ caption: value }), placeholder: __('Add caption…') }))] }), isSelected && (_jsx(BlockControls, { children: _jsxs(ToolbarGroup, { children: [_jsx(MediaReplaceFlow, { mediaId: id, mediaURL: url, allowedTypes: ['image'], accept: "image/*", onSelect: onSelectImage, onSelectURL: onSelectURL, onError: onUploadError, name: !url ? __('Add image') : __('Replace') }), _jsx(ToolbarButton, { ref: linkButtonRef, icon: link, label: href ? __('Edit link', 'jankx') : __('Link', 'jankx'), onClick: startEditing, isPressed: !!href }), href && (_jsx(ToolbarButton, { icon: linkOff, label: __('Unlink', 'jankx'), onClick: unlink }))] }) })), isEditingURL && (_jsx(Popover, { anchor: linkButtonRef?.current || popoverAnchor, placement: "bottom", onClose: () => {
                    ...
                    ], showSuggestions: true, showInitialSuggestions: true }) })), _jsxs(InspectorControls, { children: [_jsxs(PanelBody, { title: __('Preset'), initialOpen: true, children: [_jsx(SelectControl, { label: __('Layout Preset'), value: preset || '', options: [
                                    { label: __('None', 'jankx'), value: '' },
                                    ...Object.values(presets).map((p) => ({
                                        label: p.label,
                                        value: p.id
                                    }))
                                ], onChange: handlePresetChange, help: __('Choose a preset layout for the image box', 'jankx') }), currentPreset && currentPreset.description && (_jsx("p", { style: { fontSize: '12px', color: '#757575', marginTop: '8px' }, children: currentPreset.description })), currentPreset && currentPreset.options && currentPreset.options.length > 0 && (_jsxs("div", { style: { marginTop: '16px' }, children: [_jsx("strong", { style: { display: 'block', marginBottom: '12px' }, children: __('Preset Options', 'jankx') }), currentPreset.options.map(renderPresetOption)] }))] }), _jsxs(PanelBody, { title: __('Image Settings', 'jankx'), initialOpen: true, children: [_jsx(TextControl, { label: __('Image URL', 'jankx'), value: url || '', onChange: (value) => setAttributes({ url: value, id: undefined }), placeholder: __('https://…', 'jankx') }), _jsxs("div", { style: { marginBottom: '12px' }, children: [_jsx(MediaReplaceFlow, { mediaId: id, mediaURL: url, allowedTypes: ['image'], accept: "image/*", onSelect: onSelectImage, onSelectURL: onSelectURL, onError: onUploadError, name: !url ? __('Add image') : __('Replace') }), url && (_jsx(Button, { isSecondary: true, onClick: () => setAttributes({ url: undefined, id: undefined, alt: undefined, title: undefined }), style: { marginLeft: '8px' }, children: __('Remove image') }))] }), _jsx(RichText, { className: "wp-block-jankx-advanced-image-box__alt-text", tagName: "p", value: alt || '', onChange: (value) => setAttributes({ alt: value }), placeholder: __('Add alt text…'), help: __('Describe the purpose of the image. Leave empty if decorative.') }), _jsx(RichText, { className: "wp-block-jankx-advanced-image-box__title", tagName: "p", value: title || '', onChange: (value) => setAttributes({ title: value }), placeholder: __('Add title…'), help: __('Describe the role of this image on the page.') })] }), _jsxs(PanelBody, { title: __('Link Settings', 'jankx'), initialOpen: true, children: [_jsx(LinkControl, { value: { url: href || '', opensInNewTab }, onChange: ({ url: newURL = '', opensInNewTab: newTab }) => {
                                    setAttributes({ href: newURL });
                                    if (opensInNewTab !== newTab) {
                                        onToggleOpenInNewTab(!!newTab);
                                    }
                                }, onRemove: unlink, settings: [
                                    {
                                        id: 'opensInNewTab',
                                        title: __('Open in new tab', 'jankx'),
                                    },
                                ], showSuggestions: true, showInitialSuggestions: true }), href && (_jsx("div", { style: { marginTop: '16px' }, children: _jsx(TextControl, { label: __('Link Rel', 'jankx'), value: rel || '', onChange: (value) => setAttributes({ rel: value }), help: __('Add rel attribute for SEO and security (e.g. nofollow).', 'jankx') }) }))] }), _jsxs(PanelBody, { title: __('Overlay Settings', 'jankx'), initialOpen: false, children: [_jsx(ToggleControl, { label: __('Show overlay on hover'), checked: showOverlayOnHover, onChange: (value) => setAttributes({ showOverlayOnHover: value }) }), showOverlayOnHover && (_jsxs(_Fragment, { children: [_jsx(SelectControl, { label: __('Animation'), value: overlayAnimation, options: [
