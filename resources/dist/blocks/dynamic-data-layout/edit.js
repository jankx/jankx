...
function Edit({ attributes, setAttributes, clientId }) {
    const { queryPreset = 'custom', postType = 'post', useMultiPostType = false, postTypes = [], postsPerPage = 10, layout = 'grid', columns = 3, columnsTablet = 2, columnsMobile = 1, responsiveColumns, includeStickyPosts = false, orderBy = 'date', order = 'DESC', queryId, enablePagination = false, paginationStyle = 'numbers', paginationAlignment = 'center', showPaginationNumbers = true, paginationPrevText = '', paginationNextText = '', offset = 0, renderOffset = 0, renderLimit = 0, taxQuery = [], metaQuery = [], keyword = '', authorIn = [], authorNotIn = [], postIn = [], postNotIn = [], metaKey = '', metaType = '', postStatus = ['publish'], postParent = 0, postParentIn = [], postParentNotIn = [], customQueryId = '', slidesToScroll = 1, loop = false, autoplay = false, autoplayDelay = 3000, showArrows = true, showDots = true, carouselAlign = 'start', carouselAxis = 'x', carouselDirection = 'ltr', carouselStartIndex = 0, carouselDuration = 25, carouselDragFree = false, carouselDragThreshold = 10, carouselSkipSnaps = false, carouselContainScroll = 'trimSnaps', carouselInViewThreshold = 0, showTitle = true, showExcerpt = true, showFeaturedImage = true, thumbnailPosition = 'top', imageRatio = '', showDate = true, showAuthor = false, showPrice = true, showAddToCart = true, showRating = false, excerptLength = 55, spaceBetween = 16, } = attributes;
    ...
    useEffect(() => {
        const fetchTaxonomiesAndAuthors = async () => {
            ...
            try {
                ...
                const taxArray = Object.values(taxonomiesData || {}).filter((item) => typeof item?.slug === 'string' && typeof item?.name === 'string').map(item => ({
                    slug: item.slug,
                    name: item.name,
    ...
    const fetchTermsForTaxonomy = useCallback(async (taxonomySlug) => {
    ...
    if (styleColor) {
        ...
        if (bg)
            editorStyle.backgroundColor = bg;
        if (text)
            editorStyle.color = text;
        if (grad)
            editorStyle.background = grad;
    }
    ...
    const renderSettingsControl = (setting, index) => {
        ...
        switch (setting.type) {
            case 'text':
                return (_jsx(TextControl, { ...commonProps, value: attributes[setting.name] || setting.default || '', onChange: (value) => setAttributes({ [setting.name]: value }) }));
            case 'number':
                return (_jsx(TextControl, { ...commonProps, type: "number", value: attributes[setting.name] || setting.default || '', onChange: (value) => setAttributes({ [setting.name]: Number(value) }) }));
            case 'range':
                return (_jsx(RangeControl, { ...commonProps, value: attributes[setting.name] || setting.default, onChange: (value) => setAttributes({ [setting.name]: value }), min: setting.min, max: setting.max, step: setting.step }));
            case 'toggle':
                return (_jsx(ToggleControl, { ...commonProps, checked: attributes[setting.name] ?? setting.default, onChange: (value) => setAttributes({ [setting.name]: value }) }));
            case 'select':
                return (_jsx(SelectControl, { ...commonProps, value: attributes[setting.name] || setting.default, options: setting.options || [], onChange: (value) => setAttributes({ [setting.name]: value }) }));
            case 'panel':
                return (_jsx(PanelBody, { title: setting.title, initialOpen: setting.initialOpen, children: setting.controls?.map((childSetting, childIndex) => renderSettingsControl(childSetting, childIndex)) }, index));
            default:
                return null;
        }
    };
    ...
    const normalizedPresets = useMemo(() => normalizeQueryPresets(window.jankxQueryOptions?.queryPresets), []);
    ...
    return (_jsxs(_Fragment, { children: [_jsxs(InspectorControls, { group: "settings", children: [_jsxs(PanelBody, { title: __('Query Settings', 'jankx'), initialOpen: true, children: [_jsx(SelectControl, { label: __('Query Preset', 'jankx'), value: queryPreset, options: queryPresetOptions, onChange: (value) => {
                                    setAttributes({ queryPreset: value });
                                }, help: queryPresetHelp }), !useMultiPostType ? (_jsx(SelectControl, { label: __('Post Type', 'jankx'), value: postType, options: postTypeOptions, onChange: (value) => {
                                    setAttributes({ postType: value, useMultiPostType: false });
                                }, help: queryPreset === 'default' ? __('Select post type for the main query', 'jankx') : undefined })) : null, _jsx(ToggleControl, { label: __('Multi Post Type', 'jankx'), checked: useMultiPostType, onChange: (value) => {
                                ...
                                }, help: __('Enable selecting multiple post types', 'jankx') }), useMultiPostType ? (_jsx(BaseControl, { label: __('Post Types (Multiple)', 'jankx'), help: __('Select multiple post types to include', 'jankx'), children: _jsx(FormTokenField, { value: postTypes.map((slug) => {
                                        const found = postTypeOptions.find((opt) => opt.value === slug);
                                        return found?.label || slug;
                                    }), suggestions: postTypeOptions.map((opt) => opt.label), onChange: (tokens) => {
                                    ...
                                    } }) })) : null, _jsx(RangeControl, { label: __('Posts Per Page', 'jankx'), value: postsPerPage, onChange: (value) => setAttributes({ postsPerPage: value || 10 }), min: 1, max: 50, help: __('Number of posts to display', 'jankx') }), postType === 'post' ? (_jsx(ToggleControl, { label: __('Include Sticky Posts', 'jankx'), checked: includeStickyPosts, onChange: (value) => setAttributes({ includeStickyPosts: value }), help: __('Include sticky posts in the query (disabled by default).', 'jankx') })) : null, (() => {
                                const shouldRender = queryPreset !== 'default';
                                if (!shouldRender) {
                                    return null;
                                }
                                return (_jsxs(_Fragment, { children: [_jsx(SelectControl, { label: __('Order By', 'jankx'), value: orderBy, options: orderByOptions, onChange: (value) => {
                                            ...
                                            }, help: __('Sort posts by which criteria', 'jankx') }), _jsx(SelectControl, { label: __('Order', 'jankx'), value: order, options: orderOptions, onChange: (value) => {
                                                setAttributes({ order: value });
                                            } })] }));
                            })()] }), _jsxs(PanelBody, { title: __('Layout', 'jankx'), initialOpen: true, children: [_jsx(SelectControl, { label: __('Layout Type', 'jankx'), value: layout, options: layoutOptions.length > 0 ? layoutOptions : [
                                ...
                                } }), supportedOptions.includes('columns') ? (_jsx(ResponsiveControl, { label: __('Columns', 'jankx'), values: resolvedResponsiveColumns, onChange: (values) => setAttributes({
                                ...
                                } })) : null, _jsx(RangeControl, { label: __('Space Between', 'jankx'), value: spaceBetween, onChange: (value) => setAttributes({ spaceBetween: value || 0 }), min: 0, max: 100, help: __('Space between items in pixels', 'jankx') }), settingsDefinition.filter(s => s.type !== 'panel').map((setting, index) => renderSettingsControl(setting, index))] }), settingsDefinition.filter(s => s.type === 'panel').map((setting, index) => renderSettingsControl(setting, index)), _jsxs(PanelBody, { title: __('Display Slicing', 'jankx'), initialOpen: false, children: [_jsx(TextControl, { label: __('Render Offset', 'jankx'), type: "number", value: renderOffset, onChange: (value) => setAttributes({ renderOffset: parseInt(value) || 0 }), help: __('Skip first N posts from the result set (useful for breaking main query into multiple layouts)', 'jankx') }), _jsx(TextControl, { label: __('Render Limit', 'jankx'), type: "number", value: renderLimit, onChange: (value) => setAttributes({ renderLimit: parseInt(value) || 0 }), help: __('Only render N posts from the result set. Set to 0 to show all.', 'jankx') })] }), queryPreset === 'custom' ? (_jsxs(PanelBody, { title: __('Query Parameters', 'jankx'), initialOpen: false, children: [_jsx(RangeControl, { label: __('Offset', 'jankx'), value: offset, onChange: (value) => setAttributes({ offset: value || 0 }), min: 0, max: 50, help: __('Skip the first N posts', 'jankx') }), (orderBy === 'meta_value' || orderBy === 'meta_value_num') ? (_jsxs(_Fragment, { children: [_jsx(TextControl, { label: __('Meta Key', 'jankx'), value: metaKey, onChange: (value) => setAttributes({ metaKey: value }), help: __('Meta key for sorting (required when using meta_value)', 'jankx'), placeholder: __('Example: price, views, rating', 'jankx') }), orderBy === 'meta_value' ? (_jsx(SelectControl, { label: __('Meta Type', 'jankx'), value: metaType, options: window.jankxQueryOptions?.metaTypes || [
                                            { label: __('-- Auto --', 'jankx'), value: '' },
                                            { label: 'NUMERIC', value: 'NUMERIC' },
                                        ], onChange: (value) => setAttributes({ metaType: value }), help: __('Specify data type for accurate sorting', 'jankx') })) : null] })) : null] })) : null, queryPreset === 'custom' ? (_jsxs(PanelBody, { title: __('🔧 Advanced Query Parameters', 'jankx'), initialOpen: false, children: [_jsx(TextControl, { label: __('Query ID', 'jankx'), value: customQueryId, onChange: (value) => setAttributes({ customQueryId: value }), help: __('Set a name for this query to apply final filters: jankx/post-layout/query-args/{query_id}', 'jankx'), placeholder: __('Example: featured-posts, sidebar-posts', 'jankx') }), _jsx(BaseControl, { label: __('Post Status', 'jankx'), help: __('Post status to fetch (default: publish)', 'jankx'), children: _jsx(FormTokenField, { value: postStatus, suggestions: ['publish', 'pending', 'draft', 'auto-draft', 'future', 'private', 'inherit', 'trash', 'any'], onChange: (tokens) => setAttributes({ postStatus: normalizeTokens(tokens) }) }) }), _jsx(TextControl, { label: __('Post Parent ID', 'jankx'), type: "number", value: postParent, onChange: (value) => setAttributes({ postParent: parseInt(value) || 0 }), help: __('Filter posts by parent ID (0 = all)', 'jankx') }), _jsx(TextControl, { label: __('Post Parent IDs (Include)', 'jankx'), value: postParentIn.join(', '), onChange: (value) => {
                                    ...
                                    } }) })] })) : null, queryPreset === 'custom' ? (_jsxs(PanelBody, { title: __('🔢 Post ID Filters', 'jankx'), initialOpen: false, children: [_jsx(TextControl, { label: __('Post IDs (Include)', 'jankx'), value: postIn.join(', '), onChange: (value) => {
                                    const ids = value.split(',').map(v => parseInt(v.trim())).filter(v => !isNaN(v));
                                ...
                                }, help: __('Exclude posts with these IDs (comma separated)', 'jankx'), placeholder: __('Example: 4, 5, 6', 'jankx') })] })) : null, queryPreset === 'custom' ? (_jsxs(PanelBody, { title: __('⚙️ Meta Query Filters', 'jankx'), initialOpen: false, children: [_jsx(Button, { variant: "primary", onClick: () => {
                                    ...
                                    setAttributes({ metaQuery: newMetaQuery });
                                            ...
                                            setAttributes({ metaQuery: newMetaQuery });
                                        }, placeholder: __('Enter value...', 'jankx') })) : null, _jsx(SelectControl, { label: __('Type (Optional)', 'jankx'), value: mq.type || '', options: [
                                            { label: __('-- Auto --', 'jankx'), value: '' },
                                            { label: 'NUMERIC', value: 'NUMERIC' },
                                            ...
                                            setAttributes({ metaQuery: newMetaQuery });
                                        }, help: __('Specify data type for accurate comparison', 'jankx') })] }, index)))] })) : null, queryPreset === 'custom' && taxonomies.length > 0 ? taxonomies.map((taxonomy) => {
                                ...
                                if (isOpen) {
                                    ...
                                    fetchTermsForTaxonomy(taxonomy.slug);
                                            ...
                                            setAttributes({ taxQuery: newTaxQuery });
                                                    ...
                                                    setAttributes({ taxQuery: newTaxQuery });
                                                } }) })) : (_jsx(Spinner, {})) })) : null, _jsx(Button, { isDestructive: true, variant: "secondary", onClick: () => {
                                            const newTaxQuery = taxQuery.filter((_, i) => i !== existingQueryIndex);
                                            setAttributes({ taxQuery: newTaxQuery });
                                        }, style: { marginTop: '10px' }, children: __('Remove Filter', 'jankx') })] })) : null) }, taxonomy.slug));
                    }) : null, _jsxs(PanelBody, { title: __('Display Options', 'jankx'), initialOpen: false, children: [supportedOptions.includes('showFeaturedImage') && (_jsxs(_Fragment, { children: [_jsx(ToggleControl, { label: __('Show Featured Image', 'jankx'), checked: showFeaturedImage, onChange: (value) => setAttributes({ showFeaturedImage: value }), disabled: readOnlyOptions.includes('showFeaturedImage') }), showFeaturedImage && (_jsxs(_Fragment, { children: [_jsx(SelectControl, { label: __('Thumbnail Position', 'jankx'), value: thumbnailPosition, options: [
                                                    ...
                                                    { label: __('Right', 'jankx'), value: 'right' },
                                                ], onChange: (value) => setAttributes({ thumbnailPosition: value }), help: __('Position of the featured image relative to content', 'jankx') }), isCustomImageRatio ? (_jsxs("div", { style: { display: 'flex', gap: '10px', alignItems: 'flex-end', marginBottom: '1em' }, children: [_jsx("div", { style: { flex: 1 }, children: _jsx(SelectControl, { label: __('Image Ratio', 'jankx'), value: imageRatioSelectValue, options: [
                                                            ...
                                                            } }) }), _jsx("div", { style: { flex: 1 }, children: _jsx(TextControl, { label: __('Custom Ratio', 'jankx'), value: customImageRatioValue, onChange: (value) => {
                                                                const ratioPattern = /^\d+\/\d+$/;
                                                                if (!value || ratioPattern.test(value)) {
                                                                    setAttributes({ imageRatio: value || '' });
                                                                }
                                                            }, placeholder: "16/9" }) })] })) : (_jsx(SelectControl, { label: __('Image Ratio', 'jankx'), value: imageRatioSelectValue, options: [
                                                    { label: __('Original', 'jankx'), value: '' },
                                                    { label: '16:9', value: '16/9' },
                                                    { label: '4:3', value: '4/3' },
                                                    { label: '1:1', value: '1/1' },
                                                ...
                                                }, help: __('Aspect ratio for the featured image', 'jankx') }))] }))] })), supportedOptions.includes('showTitle') && (_jsx(ToggleControl, { label: __('Show Title', 'jankx'), checked: showTitle, onChange: (value) => setAttributes({ showTitle: value }), disabled: readOnlyOptions.includes('showTitle') })), !isProduct && supportedOptions.includes('showExcerpt') && (_jsxs(_Fragment, { children: [_jsx(ToggleControl, { label: __('Show Excerpt', 'jankx'), checked: showExcerpt, onChange: (value) => setAttributes({ showExcerpt: value }), disabled: readOnlyOptions.includes('showExcerpt') }), showExcerpt && (_jsx(RangeControl, { label: __('Excerpt Length', 'jankx'), value: excerptLength, onChange: (value) => setAttributes({ excerptLength: value || 55 }), min: 10, max: 200, help: __('Number of characters to display in excerpt', 'jankx') }))] })), !isProduct && supportedOptions.includes('showDate') && (_jsx(ToggleControl, { label: __('Show Date', 'jankx'), checked: showDate, onChange: (value) => setAttributes({ showDate: value }), disabled: readOnlyOptions.includes('showDate') })), supportedOptions.includes('showAuthor') && (_jsx(ToggleControl, { label: __('Show Author', 'jankx'), checked: showAuthor, onChange: (value) => setAttributes({ showAuthor: value }), disabled: readOnlyOptions.includes('showAuthor') })), hasCommerceFeatures && (_jsxs(_Fragment, { children: [_jsx(ToggleControl, { label: __('Show Price', 'jankx'), checked: showPrice, onChange: (value) => setAttributes({ showPrice: value }) }), _jsx(ToggleControl, { label: __('Show Add To Cart Button', 'jankx'), checked: showAddToCart, onChange: (value) => setAttributes({ showAddToCart: value }) }), _jsx(ToggleControl, { label: __('Show Rating', 'jankx'), checked: showRating, onChange: (value) => setAttributes({ showRating: value }) })] })), _jsx(ToggleControl, { label: __('Enable Pagination', 'jankx'), checked: enablePagination, onChange: (value) => setAttributes({ enablePagination: value }), help: __('Display pagination to paginate posts', 'jankx') }), enablePagination ? (_jsxs(_Fragment, { children: [_jsx(SelectControl, { label: __('Pagination Style', 'jankx'), value: paginationStyle, options: [
                    ...
                    if (!hasTemplateBlock) {
                        return (_jsxs("div", { style: {
                                ...
                                backgroundColor: '#f0f6fc',
                                        ...
                                        fontWeight: '600',
                                    }, children: __('Add Dynamic Data Template to define item layout', 'jankx') }), _jsx(InnerBlocks, { allowedBlocks: ['jankx/dynamic-data-template', 'core/heading'], templateLock: false, renderAppender: InnerBlocks.ButtonBlockAppender })] }));
                    }
                    return (_jsx(InnerBlocks, { allowedBlocks: ['jankx/dynamic-data-template', 'core/heading'], templateLock: false, renderAppender: InnerBlocks.DefaultBlockAppender }));
                })() })] }));
}
export default Edit;
