...
class SelectDropdown extends Component {
    ...
    dropdownOptions() {
        ...
        return this.props.options
            ...
            .map((item, index) => {
            if (!item) {
                return _jsx(DropdownSeparator, {}, 'dropdown-separator-' + index);
            }
            if (item.isLabel) {
                return _jsx(DropdownLabel, { children: item.label }, 'dropdown-label-' + index);
            }
            return (_jsx(DropdownItem, { ref: this.setItemRef(refIndex++), selected: this.state.selected === item.value, onClick: this.onSelectItem(item), path: item.path, icon: item.icon, secondaryIcon: item.secondaryIcon, count: item.count, children: item.label }, 'dropdown-item-' + item.value));
        });
    }
    render() {
        ...
        return (_jsx("div", { id: this.props.id, style: this.props.style, className: dropdownClassName, children: _jsxs("div", { ref: this.dropdownContainerRef, className: "select-dropdown__container", onKeyDown: this.navigateItem, tabIndex: this.props.tabIndex || 0, role: "button", "aria-haspopup": "true", "aria-owns": 'select-submenu-' + this.instanceId, "aria-controls": 'select-submenu-' + this.instanceId, "aria-expanded": this.state.isOpen, "aria-disabled": this.props.disabled, "data-tip-target": this.props.tipTarget, onClick: this.toggleDropdown, children: [_jsxs("div", { id: 'select-dropdown-' + this.instanceId, className: "select-dropdown__header", children: [_jsxs("span", { className: "select-dropdown__header-text", "aria-label": this.props.ariaLabel, children: [!positionSelectedSecondaryIconOnRight && selectedSecondaryIcon, selectedIcon, selectedText] }), 'number' === typeof this.props.selectedCount && (_jsx(Count, { count: this.props.selectedCount })), positionSelectedSecondaryIconOnRight && selectedSecondaryIcon, _jsx(Gridicon, { icon: "chevron-down", size: 18 })] }), _jsx("ul", { id: 'select-submenu-' + this.instanceId, className: "select-dropdown__options", role: "listbox", "aria-labelledby": 'select-dropdown-' + this.instanceId, "aria-expanded": this.state.isOpen, children: this.dropdownOptions() })] }) }));
    }
    toggleDropdown = () => {
...
export const SelectDropdownForwardingRef = forwardRef((props, ref) => (_jsx(SelectDropdown, { ...props, innerRef: ref })));
