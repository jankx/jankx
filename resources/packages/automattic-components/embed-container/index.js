...
function embedCarousel(domNode) {
    ...
    if (carouselItemsWrapper) {
        ...
        if (carouselItems && carouselItems.length) {
            createRoot(domNode).render(_jsx(DotPager, { children: carouselItems.map((item, index) => {
                    return (_jsx("div", { className: clsx('carousel-slide', item?.className), 
                        // eslint-disable-next-line react/no-danger
                        dangerouslySetInnerHTML: { __html: item?.innerHTML } }, index));
                }) }));
        }
    }
}
...
function embedTiledGallery(domNode) {
    ...
    if (galleryItems && galleryItems.length) {
        ...
        createRoot(domNode).render(_jsx("div", { className: "gallery-container", children: imageItems.map((item) => {
                const itemImage = item.querySelector('img');
                const itemLink = item.querySelector('a');
                const imageElement = (_jsx("img", { id: itemImage?.id || undefined, className: itemImage?.className || undefined, alt: itemImage?.alt || '', src: itemImage?.src || undefined, srcSet: itemImage?.srcSet || undefined }));
                return (_jsx("figure", { className: "gallery-item", children: _jsx("div", { className: "gallery-item-wrapper", children: itemLink?.href ? _jsx("a", { href: itemLink.href, children: imageElement }) : imageElement }) }));
            }) }));
    }
}
/**
