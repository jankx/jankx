...
function chooseSecondary(primary, prefered, el, target, w, h) {
    ...
    for (let i = 0, len = order.length; i < len; i++) {
        ...
        const yVisible = Math.min(off.top < viewport.top ? offBottom - viewport.top : viewport.bottom - off.top, h);
        const xVisible = Math.min(off.left < viewport.left ? offRight - viewport.left : viewport.right - off.left, w);
...
export function offset(pos, el, target, relativePosition) {
    ...
    switch (pos) {
        case 'top':
            _pos = {
                top: to.top - eh,
                left: relativePosition && relativePosition.left
