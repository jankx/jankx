...
function Edit({ attributes, setAttributes, context }: any) {
    ...
    const layoutsData = (window as any).jankxTermLayouts || {
        layoutsByTaxonomy: {},
        commonLayouts: [],
    };
    ...
    const layoutOptions = useMemo(() => {
        ...
        const specific = (layoutsData.layoutsByTaxonomy || {})[postType] || [];
        const merged = [...common];
        const names = new Set(merged.map((l: any) => l.name));
        for (const l of specific) {
            if (!names.has(l.name)) {
                merged.push(l);
            }
        }
        return merged.map((l: any) => ({ label: l.title || l.name, value: l.name }));
    ...
    const blockProps = useBlockProps({
        className: `term-layout-template term-layout-template--${templateSlug.replace('/', '-')}`,
        ...(imageRatio && { 'data-image-ratio': imageRatio }),
    });

    useEffect(() => {
        const ajaxUrlRaw = window.jankxTermLayoutTemplate?.ajaxUrl || '/wp-admin/admin-ajax.php';
        const nonce = window.jankxTermLayoutTemplate?.nonce || '';
        ...
        params.append('action', 'jankx_term_layout_preview');
            ...
            <InspectorControls>
                ...
                <PanelBody title={__('Scroll Animation', 'jankx')} initialOpen={false}>
                    <SelectControl
                        ...
                        value={(animationType || 'none') as any}
                        options={[
                            { label: __('None', 'jankx'), value: 'none' },
                        ...
                        onChange={(value: string) => setAttributes({ animationType: value as any })}
                    />
                    {animationType !== 'none' && (
                        <>
                            <RangeControl
