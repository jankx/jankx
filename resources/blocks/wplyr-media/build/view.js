    ...
    mutations.forEach(function (mutation) {
      if (mutation.type === 'childList' && mutation.addedNodes.length > 0) {
        const hasNewExternalPlayers = Array.from(mutation.addedNodes).some(node => {
          if (node.nodeType === Node.ELEMENT_NODE) {
            return node.querySelector('.jankx-wplyr-player[data-media-type="youtube"], .jankx-wplyr-player[data-media-type="vimeo"]') || node.classList.contains('jankx-wplyr-player') && (node.dataset.mediaType === 'youtube' || node.dataset.mediaType === 'vimeo');
          }
          return false;
        });
