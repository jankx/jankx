(function(){
  ...
  function isMobile(bp){ return window.innerWidth <= bp; }
  ...
  function initMegaMenu(nav){
    ...
    // Responsive toggle
    if(toggle){
      var updateToggleAria = function(){ toggle.setAttribute('aria-expanded', (!nav.classList.contains('is-collapsed')).toString()); };
      ...
      window.addEventListener('resize', function(){ setCollapsed(isMobile(breakpoint)); });
      toggle.addEventListener('click', function(e){ e.preventDefault(); nav.classList.toggle('is-collapsed'); updateToggleAria(); });
      toggle.addEventListener('keydown', function(e){ if(e.key==='Enter' || e.key===' '){ e.preventDefault(); nav.classList.toggle('is-collapsed'); updateToggleAria(); }});
    }
    ...
    qsa(listRoot || nav, '.mega-item').forEach(function(li){
      ...
      // Desktop hover
      li.addEventListener('mouseenter', function(){ if(!isMobile(breakpoint)){ closeAllSubmenus(nav); openSubmenu(li); }});
      li.addEventListener('mouseleave', function(){ if(!isMobile(breakpoint)){ li.classList.remove('mega-item--active'); link.setAttribute('aria-expanded','false'); }});
      ...
      link.addEventListener('keydown', function(e){
        ...
        if(e.key === 'Escape'){ li.classList.remove('mega-item--active'); link.setAttribute('aria-expanded','false'); link.focus(); }
      });
    });
