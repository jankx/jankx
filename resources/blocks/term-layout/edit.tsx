...
const normalizeLayoutsData = (raw: unknown): LayoutsData => {
    ...
    const layoutsByTaxonomy: Record<string, LayoutInfo[]> = {};
    if (obj.layoutsByPostType && typeof obj.layoutsByPostType === 'object') {
        Object.entries(obj.layoutsByPostType as Record<string, unknown>).forEach(([taxonomy, layouts]) => {
            const normalized = normalizeLayouts(layouts);
            if (normalized.length > 0) {
                layoutsByTaxonomy[taxonomy] = normalized;
            }
        });
    }
    return {
...
function Edit({ attributes, setAttributes, clientId, isSelected = false }: EditProps) {
    ...
    const templateBlockAttrs = useMemo(() => {
        ...
        const templateBlock = innerBlocks.find((b: any) => b?.name === 'jankx/term-layout-template');
        if (!templateBlock) {
            return null;
        }
    ...
    const layoutsData: LayoutsData = normalizeLayoutsData(window.jankxTermLayouts);
