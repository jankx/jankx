/******/ (() => { // webpackBootstrap
/*!*****************************************!*\
  !*** ./blocks/menu-builder/frontend.js ***!
  \*****************************************/
/**
  ...
  class MenuBuilderController {
    ...
    setupMobileMenuToggle() {
      ...
      if (!$('.jankx-menu-toggle').length) {
        const toggleButton = $('<button class="jankx-menu-toggle" aria-label="' + jankxMenuBuilder.i18n.openMenu + '">' + '<span class="menu-toggle-icon"></span>' + '</button>');
      ...
      if (this.mmenuInstance) {
        if (this.stateManager && this.stateManager.getState) {
          ...
          if (this.mmenuInstance.isOpen && this.mmenuInstance.isOpen()) {
            ...
            $('.jankx-menu-toggle').removeClass('is-active').attr('aria-label', jankxMenuBuilder.i18n.openMenu);
          } else {
            this.mmenuInstance.open();
            $('.jankx-menu-toggle').addClass('is-active').attr('aria-label', jankxMenuBuilder.i18n.closeMenu);
          }
        }
      }
    }
        ...
        if ($(window).width() < 1024) {
          $submenu.find('.submenu-list').each(function () {
            ...
            if (!$list.find('.submenu-back').length) {
              const backButton = $('<li class="submenu-back">' + '<a href="#" class="submenu-back-link">' + '<span class="back-arrow">←</span>' + '<span class="back-text">' + jankxMenuBuilder.i18n.backToParent + '</span>' + '</a>' + '</li>');
