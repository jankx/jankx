...
interface QueryInspectorControlsProps {
	attributes: {
		query: {
			order: string;
			orderBy: string;
			...
			sticky: string;
			inherit: boolean;
			taxQuery: Record<string, any> | null;
...
export default function QueryInspectorControls(props: QueryInspectorControlsProps) {
	...
	const onPostTypeChange = (newValue: string) => {
		const updateQuery: Record<string, any> = { postType: newValue };
	...
	const debouncedQuerySearch = useMemo(() => {
		return debounce((newQuerySearch: string) => {
